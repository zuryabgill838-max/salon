...
const readStorage = (keys: string[]): RecordItem[] => {
...
const getDate = (item: RecordItem) =>
  item.date || item.appointmentDate || item.bookingDate || item.createdAt || "";

export default function Dashboard() {
  const [appointments, setAppointments] = useState<RecordItem[]>([]);
  const [customers, setCustomers] = useState<RecordItem[]>([]);
  const [workers, setWorkers] = useState<RecordItem[]>([]);

  useEffect(() => {
    const loadDashboardData = () => {
      setAppointments(readStorage(["appointments", "salon_appointments", "bookings"]));
      setCustomers(readStorage(["customers", "salon_customers"]));
      setWorkers(readStorage(["workers", "staff", "salon_workers"]));
    };

    loadDashboardData();
    window.addEventListener("storage", loadDashboardData);
    window.addEventListener("salon-data-updated", loadDashboardData);
    return () => {
      window.removeEventListener("storage", loadDashboardData);
      window.removeEventListener("salon-data-updated", loadDashboardData);
    };
  }, []);

  const today = new Date().toISOString().slice(0, 10);
  const todaysAppointments = useMemo(
    () => appointments.filter((item) => String(getDate(item)).slice(0, 10) === today),
    [appointments, today]
  );
  const activeWorkers = useMemo(
    () => workers.filter((item) => item.active !== false && item.status !== "inactive"),
    [workers]
  );
  const todaysRevenue = useMemo(
    () => todaysAppointments.reduce((sum, item) => {
      const amount = Number(item.price ?? item.amount ?? item.total ?? item.servicePrice ?? 0);
      return sum + (Number.isFinite(amount) ? amount : 0);
    }, 0),
    [todaysAppointments]
  );
  const recentAppointments = useMemo(
    () => [...todaysAppointments].sort((a, b) => String(getDate(a)).localeCompare(String(getDate(b)))).slice(0, 5),
    [todaysAppointments]
  );
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 text-sm text-gray-500">
                Live count from saved appointments
              </p>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 text-sm text-gray-500">
                Calculated from today's appointments
              </p>
